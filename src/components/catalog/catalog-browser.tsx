...
export function CatalogBrowser({
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
      <div className="sticky top-16 z-30 -mx-4 mb-8 border-b border-zinc-200 bg-white/90 px-4 py-4 backdrop-blur sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-zinc-200 bg-white py-3 pl-10 pr-4 text-sm text-zinc-800 outline-none transition-colors placeholder:text-zinc-400 focus:border-brand-400 focus:ring-2 focus:ring-brand-100"
            />
          </div>
          <div className="flex items-center gap-2 text-sm text-zinc-500 lg:shrink-0">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((tpl) => {
            ...
            return (
              <article
                key={tpl.id}
                className={`group relative flex flex-col overflow-hidden rounded-3xl border bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl ${
                  isSelected ? "border-brand-400 ring-2 ring-brand-100" : "border-zinc-200"
                }`}
              >
                {tpl.isBundle && (
                  <span className="absolute right-4 top-4 z-10 inline-flex items-center gap-1 rounded-full bg-zinc-900 px-3 py-1 text-[11px] font-bold text-white">
                  ...
                  <div className="mt-5 border-t border-zinc-100 pt-5">
                    ...
                    <div className="mt-4 flex gap-2">
                      <button
                        ...
                        className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-zinc-200 px-3 py-2.5 text-xs font-semibold text-zinc-700 transition-colors hover:bg-zinc-50"
                      >
      ...
      {selected.length > 0 && (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-zinc-200 bg-white/95 px-4 py-4 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] backdrop-blur sm:px-6 lg:px-8">
          <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 sm:flex-row">
            ...
            <div className="flex w-full gap-2 sm:w-auto">
              <button
                type="button"
                onClick={() => setSelected([])}
                className="rounded-full border border-zinc-200 px-4 py-2.5 text-sm font-medium text-zinc-600 transition-colors hover:bg-zinc-50"
              >
                Kosongkan
              </button>
              <Link
...
function PreviewModal({
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative border-b border-zinc-100 p-6 sm:p-8">
          <button
            type="button"
            onClick={onClose}
            className="absolute right-5 top-5 grid size-9 place-items-center rounded-full bg-zinc-100 text-zinc-500 transition-colors hover:bg-zinc-200"
            aria-label="Tutup"
          >
            <X className="size-4" />
          </button>
          ...
          <h3 className="mt-3 max-w-lg text-xl font-extrabold text-zinc-900 sm:text-2xl">
            {template.title}
          </h3>
        ...
        <div className="grid gap-6 p-6 sm:grid-cols-2 sm:p-8">
          ...
          <div>
            <h4 className="text-sm font-bold text-zinc-900">Format File</h4>
            <div className="mt-3 space-y-2">
              {template.formats.map((format) => (
                <div
                  key={format}
                  className="flex items-center gap-2 rounded-xl border border-zinc-200 px-3 py-2.5 text-sm text-zinc-700"
                >
        ...
        <div className="flex flex-col gap-3 border-t border-zinc-100 p-6 sm:flex-row sm:justify-end sm:p-8">
          <a
            href={waLink(
            ...
            className="inline-flex items-center justify-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-5 py-3 text-sm font-semibold text-emerald-700 transition-colors hover:bg-emerald-100"
          >
            Tanya via WhatsApp
          </a>
          <button
            ...
            className={`inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white transition-colors ${
              selected ? "bg-zinc-900 hover:bg-zinc-700" : accent.button
            }`}
          >
